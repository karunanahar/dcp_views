	   ...
	   //             $bareme = "00";
            ...
            //         }
               ...
               if (($age >= MAX_AGE_INSURANCE_RATE) || $result['loan_term'] >= MAX_LOAN_TERM) { // CR-K-1 : 02-MAR-2025 : constant added and loan term condition added
				...
				$array = array($customer_data->id_client,$result['application_no'],$type1,$cat,$loan_other_data['loan_amt'],date("Y-m-d", strtotime($result['final_disburse_date'])),date("Y-m-d", strtotime($result['created_at'])),$loan_term,"M","1",$loanDate,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$loan_other_data['loan_interest'],$bareme,$code_auth,sprintf("%02d", $loan_other_data['value_rg'] ? $loan_other_data['value_rg'] : 00),round($result['frais_de_dossier'],0),$placeur,round($teg['tegvalue'],2),$code_fcub,$customer_data->first_name,$customer_data->last_name,$customer_data->street,$customer_data->city,$customer_data->email,$customer_data->main_phone,$customer_data->nomGestionnaire,$customer_data->flexcube_code);
