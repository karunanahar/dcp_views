...
class Tokos_on_demand extends Admin_Controller 

{
	...
	public function create(){
    	...
    	$content = "ID Client;Numero de pret Digital Credit;Code Produit;Categorie de pret Flexcube;Nominal du Pret;Date Accord;Date de Realisation;Nombre Echeance;Unite Echeance;Frequence par Unite Echeance;Date de Premiere Echeance;Numero composante principale;Numero composante Interets;Numero composante TVA;Numero composante CSS;Taux Interet Fixe;Code Bareme de Assurance;Code Autorisation;Valeur RG;Frais De Dossier HT;Nom Exploitant;TEG;Prénoms;Nom Patronymique;Lieu d'habitation;Adresse postale;Ville;Email;Numéro de téléphone Portable;Nom Gestionnaire;Flexcube Code Agence"."\r\n";
        
       // $content = "ID Client;Numero de pret Digital Credit;Code Produit;Categorie de pret Flexcube;Nominal du Pret;Date Accord;Date de Realisation;Nombre Echeance;Unite Echeance;Frequence par Unite Echeance;Date de Premiere Echeance;Numero composante principale;Numero composante Interets;Taux Interet Fixe;Code Bareme de Assurance;Code Autorisation;Frais De Dossier HT;"."\r\n";

		//echo "Extract Data from ".$tdate=DATE;
		
		$tdate = strtotime('11/15/2018');

        $tdate = date('Y-m-d',$tdate);
		//$tdate=date_format($tdate,"Y-m-d");
		
        //echo $tdate;
		$file_name =  "Tokos on Demand by. ".$tdate.".txt";
		mkdir(FCPATH . "/upload_data/On_Demand_Tokos".$tdate, 0777, true);
...
//             die;
	   ...
	   //             $bareme = "00";
            ...
            //         }
               ...
               if (($age >= MAX_AGE_INSURANCE_RATE) || $result['loan_term'] >= MAX_LOAN_TERM) { // CR-K-1 : 02-MAR-2025 : constant added and loan term condition added
				...
				$array = array($customer_data->id_client,$result['application_no'],$type1,$cat,$loan_other_data['loan_amt'],date("Y-m-d", strtotime($result['final_disburse_date'])),date("Y-m-d", strtotime($result['created_at'])),$loan_term,"M","1",$loanDate,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$loan_other_data['loan_interest'],$bareme,$code_auth,sprintf("%02d", $loan_other_data['value_rg'] ? $loan_other_data['value_rg'] : 00),round($result['frais_de_dossier'],0),$placeur,round($teg['tegvalue'],2),$customer_data->first_name,$customer_data->last_name,$customer_data->street,$customer_data->city,$customer_data->email,$customer_data->main_phone,$customer_data->nomGestionnaire,$customer_data->flexcube_code);
                
              //  $array = array($customer_data->id_client,$result['application_no'],$type1,$cat,$loan_other_data['loan_amt'],date("Y-m-d", strtotime($result['final_disburse_date'])),date("Y-m-d", strtotime($result['created_at'])),$loan_term,"M","1",$loanDate,$customer_data->flexcube_acct,$customer_data->flexcube_acct,$loan_other_data['loan_interest'],$bareme,$code_auth,round($result['frais_de_dossier'],0));
	...
	//	if (!file_exists(FCPATH  . "/upload_data/App_Date".$tdate."/".$file_name)) {
         ...
         $fp = fopen(FCPATH  . "/upload_data/On_Demand_Tokos".$tdate."/".$file_name ,"w") or die('Cannot open file');

    		fwrite($fp,$content."\n");

			fclose($fp);
			
			$url=FCPATH  . "/upload_data/On_Demand_Tokos".$tdate."/".$file_name;
